 ...
 *
 * This reads the canonical mirk.css (single source of truth in mirk-interface),
...
// mirk-interface normally sits next to the hypercms checkout, but in a git
// worktree (.claude/worktrees/<name>) the repo root is nested deeper, so walk
// up until a directory contains mirk-interface/mirk.css.
function findMirkRepo(from) {
  for (let dir = from; ; dir = path.dirname(dir)) {
    const candidate = path.join(dir, 'mirk-interface')
    if (fs.existsSync(path.join(candidate, 'mirk.css'))) return candidate
    if (path.dirname(dir) === dir) throw new Error('mirk-interface/mirk.css not found above ' + from)
  }
}
...
const banner =
  '/* GENERATED by scripts/build-theme.js from mirk-interface/mirk.css — DO NOT EDIT.\n' +
