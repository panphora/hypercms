...
test('onAdd: new card has every field from item shape', () => {
  ...
  try {
    ...
    // Query leaf textareas only — since v0.3 stamps data-hcms-field on wrapping
    // containers too (option 1), a bare [data-hcms-field] would match the
    // container AND the leaf field for each field.
    const fields = Array.from(newCard.querySelectorAll('textarea[data-hcms-field]')).map(
